...
def parse_epd(
    epd: str, VariantBoard: Type[chess.Board]
) -> Tuple[chess.Board, str, List[chess.Move], List[chess.Move]]:
    ...
    if "am" in epd_info:
        ...
        description = "{} (avoid {})".format(
            description, " and ".join(board.san(m) for m in am))
    else:
        am = []
    ...
    if "bm" in epd_info:
        ...
        description = "{} (expect {})".format(
            description, " or ".join(board.san(m) for m in bm))
    else:
        bm = []
...
async def test_epd(engine: chess.engine.Protocol, epd: str,
                   VariantBoard: Type[chess.Board], movetime: float) -> float:
...
async def test_epd_with_fractional_scores(engine: chess.engine.Protocol,
                                          epd: str,
                                          VariantBoard: Type[chess.Board],
...
async def main() -> None:
    ...
    engine_group.add_argument("-u", "--uci", help="The UCI engine under test.")
    engine_group.add_argument("-x",
                              "--xboard",
                              help="The XBoard engine under test.")

    parser.add_argument("epd",
                        nargs="+",
